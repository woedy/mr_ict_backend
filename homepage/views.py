...
from students.models import StudentBadge, StudentChallenge, StudentCourse, StudentCourseLesson
...
def get_student_homepage_data_view(request):
    ...
    messages = user.messages.all().filter(read=False)
    messages_count = messages.count()


    badges = StudentBadge.object.all().filter(student__user=user)
    ...
    ### Course Overview ###
    in_progress = StudentCourse.object.filter(student__user=user, completed=False)
    completed_count = StudentCourse.object.filter(student__user=user, completed=True)
    challenges_count = StudentChallenge.object.filter(student__user=user, completed=True)
    ...
    ### Resume Learning ###
    resume = StudentCourseLesson.object.filter(student__user=user, completed=False)
    ...
    #HTML
    html_challenges = CodingChallenge.objects.filter(is_archived=False, course='HTML')[:5]
    ...
    #CSS
    css_challenges = CodingChallenge.objects.filter(is_archived=False, course='CSS')[:5]
    ...
    #Javascript
    javascript_challenges = CodingChallenge.objects.filter(is_archived=False, course='Javascript')[:5]
    ...
    #Python
    python_challenges = CodingChallenge.objects.filter(is_archived=False, course='Javascript')[:5]
